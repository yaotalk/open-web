    ...
    <Modal :title="xstitle" v-model="modal" :width=300 :mask-closable="false"
      ...
      <div class="panellite">
        ...
        <div class="form-group">
          ...
          <input type="text" v-model="xsusername" required="required" class="form-control" id="xsuser" autocomplete="off">
        </div>
        <div class="form-gruop" style="margin-bottom: 20px;">
          <label class="form-label">密 码</label>
          <input type="password" v-model="xsuserpassword" class="form-control" id="xspwd" required="required" autocomplete="off">
        </div>
      ...
      <div slot="footer" v-if="isLogin">
        ...
        <Button type="primary" size="large" long @click="loginOpe(0)">登录</Button>
      </div>
      <div slot="footer" v-if="isRegister">
        <Button type="primary" size="large" long @click="loginOpe(1)">注册</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    loginOpe: function (obj) {
      if (obj == 0) {//登录
        // this.glogingStatus = true;
        this.$http.post("api/login", "username="+this.xsusername +"&password="+this.xsuserpassword, {headers: {
        ...
        this.modal = false;
        this.$emit('on-login-success', true);
        // let data = "{\"username\":\"xiaohong\",\"userpassword\":\"123456\"}";
      // const _this = this;
      // console.log('username: === ' + _this.xsusername);
      // console.log('userpassword: === ' + _this.xsuserpassword);
      // this.$http.post(this.$Ip + '/user/login', {username:_this.xsusername,userpassword:_this.xsuserpassword})
      ...
      //     if (tmpdata == 'success') {
      //       _this.modal = false;
      //       _this.$emit('on-login-success', true);
      //     }else{
